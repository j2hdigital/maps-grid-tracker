...
export default function Home() {
  ...
  async function startGrid(e) {
    e?.preventDefault?.();
    setLoading(true);
    setRanks({}); setIds([]); setCells([]); setProgress({ done:0, total:0 });
      ...
      <div style={{ marginTop: 16, display:"grid", gridTemplateColumns: gridTemplate, gap: 6, alignItems:"center", justifyItems:"center" }}>
        {tiles.map((t, i) => (
          <div key={t.id || i}
            ...
            style={{ width:36, height:36, borderRadius:6, background: colorFor(t.rank), color:"#111", fontSize:12, display:"flex", alignItems:"center", justifyContent:"center", border:"1px solid rgba(0,0,0,0.08)" }}>
