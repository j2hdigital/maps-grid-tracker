...
function findRank(items, target) {
  ...
  for (const it of (items||[])) { if (wantPlace && it.place_id === wantPlace) return it.rank_group ?? it.rank_absolute ?? null; }
  if (wantName) for (const it of (items||[])) {
    const title = norm(it.title || it.name || "");
    if (title && (title === wantName || title.includes(wantName))) return it.rank_group ?? it.rank_absolute ?? null;
  }
  return null;
}
